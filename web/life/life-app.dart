...
const String DEFAULT_CONFIGURATION = "life/configurations/original.json";
...
class LifeApp extends PolymerElement {
	...
	void attached() {
		...
		shadowRoot.querySelector("#play_header").onClick.listen((_) => (shadowRoot.querySelector("#play_collapse") as CoreCollapse).toggle());
		shadowRoot.querySelector("#click_header").onClick.listen((_) => (shadowRoot.querySelector("#click_collapse") as CoreCollapse).toggle());
		shadowRoot.querySelector("#import_header").onClick.listen((_) => (shadowRoot.querySelector("#import_collapse") as CoreCollapse).toggle());

		HttpRequest.getString(DEFAULT_CONFIGURATION).then(importConfiguration, onError: (_) => alert("Import Error"));
	}
    ...
    void initialize() {
    	...
    	graphics = new Graphics(100, 100, config.stateColorMap, config.stateColorMap[config.defaultState]);

		shadowRoot.querySelector("#grid_container").style..minWidth = (graphics.width.toString() + "px")
		        										 ..maxWidth = (graphics.width.toString() + "px")
		        										 ..minHeight = (graphics.height.toString() + "px")
														 ..maxHeight = (graphics.height.toString() + "px");

		backgroundCanvas..width = graphics.width
						..height = graphics.height;

		foregroundCanvas..width = graphics.width
						..height = graphics.height;
